...
for (var i = 0; i < shops.length; i++) {
    ...
    $("#accordion").append(
        "<h3>" + shop.name + "</h3>" +
        "<div>" +
        "<p>Customer's We Average Per Hour:  <span>" + average(shop.customersPerHour).toFixed(0) + "</span></p>" +
...
$("form").on("submit",handleSubmitComment);
$("#comments").on("keydown", function(event){
    if(event.keyCode === 13) {
...
function handleSubmitComment(event) {
    ...
    if (name && comment) {
        $('#name').val("");
        $('#comments').val("");
        $(".comments").prepend("<div class='comment'>" + name + " : <span>" + comment + "</span></div>");
    }
}
